  ...
  try {
    ...
    if (newSlug && newSlug !== slug) {
      ...
      if (existingPackage) {
        return res
          .status(400)
          .json({ message: "A package with this slug already exists" });
      }
    }
    ...
    if (req.files?.length) {
      updatedData.overallImages = req.files
        .slice(0, 5)
        .map((file) => file.path);
      updatedData.itinerary = JSON.parse(req.body.itinerary || "[]").map(
        (day, index) => ({
          ...day,
          imageUrl: req.files[5 + index] ? req.files[5 + index].path : null,
        })
    ...
    res
      .status(500)
      .json({ message: "Failed to update package", error: err.message });
  }
};
...
const deletePackage = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to delete package", error: err.message });
  }
};
